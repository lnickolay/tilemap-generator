"""Contains a specialized PyQt widget class for float input."""
...
class FloatSpinBox(qtw.QDoubleSpinBox):
    """A QDoubleSpinBox that signals only when a committed value is new.

    This class inherits from QDoubleSpinBox but adds logic to the 'editingFinished' event. It checks if the new value
    differs from the previously committed value ('_last_value') before emitting the custom 'value_change_commited'
    signal. This ensures that signals are only emitted for meaningful value changes.

    Signals:
        value_change_commited: Emitted when the user finishes editing and the value has changed since the last commit.
    """

    value_change_commited = qtc.pyqtSignal(float)

    # The last successfully committed float value, used to determine if a change is meaningful.
    _last_value: float

    def __init__(self, default_value: float, min_value: float, max_value: float, step_size: float) -> None:
        """Initializes the specialized float spin box.
        ...
        Args:
            ...
            min_value: The lowest float value allowed in the spin box.
            max_value: The highest float value allowed in the spin box.
